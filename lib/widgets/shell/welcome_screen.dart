...
class _WelcomeScreen extends ConsumerWidget {
  ...
  List<Widget> _startColumn(
    ...
    return [
      ...
      ..._presentationImportButton(context, ref, l10n, secondaryStyle),
  ...
  /// De terugweg vanuit PowerPoint, Keynote of Impress, pal onder 'Nieuwe
  /// presentatie' — dezelfde plek die 'Document importeren…' onder 'Nieuw
  /// document' inneemt.
  ///
  /// Hij ontbrak, en dat liep dood: wie hier een `.pptx` kwam binnenhalen vond
  /// alleen de documentimport, waar een presentatie grijs staat omdat die weg
  /// naar `.docx`/`.odt` leidt. De import zat wél in het menu van de
  /// werkruimte, maar dat vraagt eerst een open presentatie — terwijl de import
  /// er juist zelf een oplevert.
  ///
  /// Zelfde poort als dat menu-item ([importModuleRevealProvider]) en als de
  /// OpenKAT-knop verderop: een module die uit staat, blijft uit. Losse methode
  /// zodat [_startColumn] onder de methodelengte-ratchet blijft.
  List<Widget> _presentationImportButton(
    BuildContext context,
    WidgetRef ref,
    AppLocalizations l10n,
    ButtonStyle secondaryStyle,
  ) {
    if (!ref.watch(importModuleRevealProvider)) return const [];
    return [
      const SizedBox(height: 8),
      _wideSecondaryButton(
        style: secondaryStyle,
        icon: Icons.upload_file_outlined,
        // Dezelfde bronstring als het menu-item: twee bijna gelijke zinnen
        // laten vertalen is hoe ze uit elkaar gaan lopen.
        label: Text(presentationImportLabel(l10n)),
        onPressed: () => importPresentation(context, ref),
      ),
    ];
  }
