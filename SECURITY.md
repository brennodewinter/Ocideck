...
a reply before disclosing anything publicly.

> **Two things about that address, stated rather than glossed over.** OciDeck is
> published by **Stichting LibreKAT** (the foundation named in *Settings → About*),
> not by Vigilis; the reporting address predates that and has not yet been moved.
> Which address is the lasting one is a decision the publisher still has to
> record here — **`<reporting address: to be confirmed by the publisher>`** —
> and until it does, the address above is the one that is monitored. Second:
> there is **no published PGP key or other encryption channel**. Earlier text
> here asked you to "encrypt the report if you can", which is not an
> instruction anyone can follow without a key. So a report reaches us as
> ordinary e-mail. If that is unacceptable for what you have found, say so in a
> first message without the details and we will agree on a channel.
...
- **Acknowledgement** of your report within
  **`<response window: to be chosen by the publisher>`**. Until that number is
  filled in, treat "as quickly as we reasonably can" as the honest description
  and the reminder route above as the fallback — an unbounded promise is not a
  promise.
...
## Where this policy stands under the Cyber Resilience Act

Regulation (EU) 2024/2847 (the Cyber Resilience Act) puts its heavy obligations
on a **manufacturer** who places a product with digital elements on the market.
Stichting LibreKAT publishes OciDeck as **open-source software** and does not
place it on the market as such a product, so those manufacturer obligations do
not apply to it by operation of law.

The foundation nevertheless **holds itself voluntarily to the CRA standards
wherever it can**, as a quality bar rather than a legal duty. Concretely, we
commit to: a machine-readable SBOM with every build; a support period with a
stated end date; secure default settings; a reporting point with a stated
response window; an update path; and vulnerability handling along the lines of
Annex I Part II.

That is the commitment. This is what is true of it **today**, which is less:

- **SBOM.** Generated by `make sbom`, kept current by a staleness gate in
  `make check`, and published in the repository. It travels with the **web**
  build only; the desktop builds do not yet carry it. See
  [Software Bill of Materials](#software-bill-of-materials).
- **Support period.** Not set. The value belongs here:
  **`<support period: to be chosen by the publisher>`**. There is no release to
  count it from yet either — see [Supported versions](#supported-versions).
- **Secure defaults.** Largely true and testable: the privacy check is on, the
  AI backend, remote media and the CVE lookup are off, and network requests pass
  a guard against internal addresses. The one loud exception is the macOS App
  Sandbox, which is off — see [Platform sandboxing](#platform-sandboxing-macos).
  The classification gate is off by default too, which is deliberate; see
  [SECURITY_DESIGN.md](docs/SECURITY_DESIGN.md) §"Standards".
- **Reporting point.** Exists (above). The response window is not yet a number.
- **Update path.** Does not exist. There is no updater, no signed build, no
  release channel, and the app does not display its own version, so a user
  cannot tell what they are running or be told that something newer exists.
  Building from source is the only route.
- **Vulnerability handling.** Coordinated disclosure as described above; the
  known-issue register lives in this file.

We deliberately do not describe OciDeck as "CRA-compliant" or as "meeting the
CRA". Several of the items above are not in place, and a compliance claim would
be false the moment one of them slipped. What we can say is what we are aiming
at and where we actually stand. *Added 2026-07-21; this section replaces the
silence where a support period and a response window should have been.*
...
This is the artefact that **Annex I Part II §1** of the Cyber Resilience Act
(Reg. (EU) 2024/2847) describes, and we keep it to that standard voluntarily —
see [Where this policy stands under the Cyber Resilience Act](#where-this-policy-stands-under-the-cyber-resilience-act).
It also feeds external vulnerability scanners (Dependency-Track, `osv-scanner`)
for the Dart/Flutter graph. See [`docs/SBOM.md`](docs/SBOM.md).
...
tagged, fixes will target the latest release plus that branch, and the support
period referred to above becomes a date rather than a placeholder.
