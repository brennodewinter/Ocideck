...
void main() {
  ...
  group('met een werkende native laag', () {
    ...
    test('een kat is geen persoon', () async {
      ...
      if (!scanner.isSupported) return;
      expect(await scanner.countFaces(asset('cat-keiko.jpg')), 0);
      expect(await scanner.countFaces(asset('cat-otis.jpg')), 0);
    });

    test('een logo is geen persoon', () async {
      if (!scanner.isSupported) return;
      expect(await scanner.countFaces(asset('ocideck-logo.png')), 0);
      expect(await scanner.countFaces(asset('librekat-logo.png')), 0);
    });
  });

  group('robuustheid — geldt met en zonder native laag', () {
    test('lege bytes geven nul, geen fout', () async {
      expect(await scanner.countFaces(Uint8List(0)), 0);
    });

    test('onleesbare bytes geven nul, geen fout', () async {
      // Een .svg of een half gedownload bestand hoort de kwaliteitscontrole
      // niet te breken.
      expect(await scanner.countFaces(Uint8List.fromList([1, 2, 3, 4, 5])), 0);
    });
    ...
    test('een absurd groot bestand wordt overgeslagen', () async {
      ...
      expect(await scanner.countFaces(Uint8List(25 * 1024 * 1024)), 0);
    });
  });
  ...
  group('de webvariant', () {
    ...
    test('telt niets, en gooit niet', () async {
      expect(await web.countFaces(Uint8List.fromList([1, 2, 3])), 0);
      web.dispose();
    });
  });
}
