...
void main() {
  ...
  group('an old actions slide migrates to a plain table', () {
    ...
    // niet geconverteerd te worden: de parser leest het token als `table` en de
    // rijen komen ongewijzigd mee.
    test('the actions class token reads back as a table, rows intact', () {
      final service = MarkdownService();
      final rows = [
        const ['Action', 'Owner', 'Due', 'Status'],
        const [
          'acc-oud uit de lucht halen',
          'Team Platform',
          '2026-08-15',
          'open',
        ],
      ];
      // Genereer een echte tabeldia en verwissel alleen het token, zodat de test
      // niet op een met de hand nagebouwd bestandsformaat leunt.
          ...
          .generateDeck(
            Deck(
              ...
              slides: [
                Slide.create(
                  SlideType.table,
                ).copyWith(title: 'Wat we vragen', tableRows: rows),
              ],
            ),
          )
          .replaceFirst('<!-- _class: table -->', '<!-- _class: actions -->');
