...
>
> Four addresses are not ones you picked, so they are named here rather than left to be discovered. **On the web**, a URL import that the browser refuses on CORS grounds is retried through a `fetch-proxy` endpoint on the origin the app was served from, which means the address you typed reaches whoever hosts that build ([HOSTING.md](docs/HOSTING.md) §4). The optional, off-by-default **CVE lookup** defaults to `cveapi.librekat.nl` — a mirror run by the publisher — and falls back to ENISA's EU vulnerability database and MITRE, neither of which is configurable; the **local CVE database** downloads its bulk data via `api.github.com`. A **video you embed** is fetched from the service that hosts it: a Vimeo embed loads Vimeo's player script from `vimeo.com`, a YouTube embed loads no script at all but does request the player and the video itself from `youtube-nocookie.com` and its media hosts. The full list, with what each one sends, is in [PRIVACY.md](docs/PRIVACY.md#what-leaves-your-device--and-only-when-you-ask); see [ARCHITECTURE.md](docs/ARCHITECTURE.md#runtime--network-model) for the mechanics. *(Corrected 2026-07-21: the earlier wording said "the only outbound calls are ones you start" and listed neither the proxy nor the publisher-run mirror. Corrected 2026-07-22: both services were described as loading a player script; since that date the YouTube embed loads none, and it never touches `youtube.com`.)*
...
- **Import / export** — round-trips Marp Markdown, imports existing slides, and exports to PDF, PPTX (with speaker notes), and an offline HTML deck: the JavaScript, CSS, font, charts **and the slide images** are inlined, so code highlighting, math, charts, diagrams and photos render in a browser with no network fetch and nothing beside the file. **In that HTML export** each image is re-encoded to screen size (so a deck of photos stays megabytes, not tens of them) and embedded once however many slides use it, and the re-encode drops its EXIF — GPS, timestamp, camera serial. Two caveats, because "EXIF is stripped" read more broadly than it is true: **GIF and SVG travel unchanged** (a GIF loses its animation on re-encode and an SVG has no pixels to scale), and the images in your own project folder and inside a `.ocideck` package **keep their original metadata** — OciDeck never rewrites your source file. Hand over the package or the `images/` folder and you hand over the EXIF with it. *(Corrected 2026-07-22: this sentence said images are re-encoded "on the way in", which reads as "on import".)* **Video is the exception**: a local video file is not inlined (size) and an embedded YouTube/Vimeo player is blocked by the export's own security policy. Decks are saved as a self-contained package with copied assets.
...
| --- | --- | --- |
| [OciWacht](docs/design/OCIWACHT.md) | **Implemented** | Detecting privacy-sensitive data in a deck, and — where you ask for it — removing it from everything you share, at one enforced boundary |
