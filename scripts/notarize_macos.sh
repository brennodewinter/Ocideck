...
# hem bewust over.
#
# `launchctl managername` is daarvoor een te grove maat: die blijft "Aqua"
# terwijl het scherm vergrendeld is, en dan komt de app wél op maar gaat ze
# meteen weer weg. Zo sloeg de release-CI van v0.6.7 rood op een binary die
# op een ontgrendelde Mac gewoon bleef draaien — getekend, genotariseerd en
# kerngezond. De sessiestatus uit ioreg ziet dat verschil wel.
# ---------------------------------------------------------------------------
# Staat het scherm op slot, of hoort deze sessie niet bij de console? Dan kan
# een Cocoa-app niet normaal opkomen. Onbekend telt hier bewust als "niet
# geblokkeerd": een onleesbare sessiestatus mag nooit een excuus worden om de
# proef stil over te slaan.
grafische_sessie_geblokkeerd() {
  local sessie
  sessie="$(ioreg -n Root -d1 -a 2>/dev/null)" || return 1
  if printf '%s' "$sessie" | grep -A1 'CGSSessionScreenIsLocked' \
      | grep -q '<true/>'; then
    return 0
  fi
  if printf '%s' "$sessie" | grep -A1 'kCGSSessionOnConsoleKey' \
      | grep -q '<false/>'; then
    return 0
  fi
  return 1
