...
**Slide** — one immutable, strongly-typed slide. Its `SlideType` (32 values)
...
`flow`, `phaseGate`, `gantt`), and the Managementsysteem layout (`controlStatus`). The seven
...
adds `controlStatus`; 2026-08-05: `gantt` added for Procesverbetering; count held by `test/docs_enum_counts_test.dart`.)*
