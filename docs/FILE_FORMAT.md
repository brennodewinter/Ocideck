...
| `<!-- ocideck_openkat_generated_origin -->` | Marks the generated original of an OpenKAT view. A manual duplicate deliberately loses this marker, so a report update can preserve the copy. Legacy decks without unambiguous provenance stop the update instead of risking user-authored content. |
