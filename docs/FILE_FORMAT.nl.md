...
| `documentHeadingFontFamily` | `null` | **Documenten.** Lettertype van de koppen van een document, uit dezelfde aangeboden lijst als `fontFamily`. `null` houdt de koppen in `fontFamily`, zoals altijd. Een dia leest het nooit: een aparte kopletter is een documentgegeven (Words *major*-letter), geen diagegeven. *(Toegevoegd 2026-09-19, #2119.)* |
| `preferredFontFamily` | `null` | De letter die de huisstijl *werkelijk* vraagt — het `Aptos` van een geïmporteerd Word-document, bijvoorbeeld — wanneer OciDeck hem zelf niet kan tonen. `fontFamily` is dan de plaatsvervanger op het scherm; een export noemt deze als eerste (de HTML-letterstapel, `w:rFonts` in Word, `style:font-name` in LibreOffice; de PDF en LaTeX nemen alleen de klasse schreef/schreefloos over), zodat een programma dat de letter heeft het echte gezicht toont. `null`: geen voorkeur, scherm en export zetten dezelfde letter. Vrije tekst, gesaneerd bij het lezen: letters, cijfers, spatie, `.` en `-`, hooguit 64 tekens, beginnend met een letter of cijfer — alles wat uit een CSS- of XML-declaratie kan breken valt terug op `null`. *(Toegevoegd 2026-09-19, #2119.)* |
| `preferredDocumentHeadingFontFamily` | `null` | **Documenten.** Als `preferredFontFamily`, voor de koppen. Bij export gebruikt een kop deze, anders `documentHeadingFontFamily`, anders de exportletter van de tekst (`preferredFontFamily` ?? `fontFamily`). *(Toegevoegd 2026-09-19, #2119.)* |
