...
| P0-AIBK | AI shared backend | — | new AI client (OpenAI-compatible `/v1`), settings toggle, 3-tier backend, guardrail scaffolding (AI_ASSIST §3–4); reuse `consent_provider`, `net_guard`, `flutter_secure_storage` | Toggle off by default; "test connection" to a local endpoint works; no egress in local mode |
