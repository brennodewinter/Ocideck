...
- `welcome_screen.dart` — `_WelcomeScreen`: recent files and new/open/import actions, plus the three things a first-time visitor needs — one line saying what OciDeck is, the number of templates behind *New presentation* (`_visibleTemplateCount`, counted from `deckTemplates` under the same `requiresInfoSafety` rule as the picker, so the promise cannot be one too many), and a direct button to the user guide beside *Settings*.
...
- `new_deck_dialog.dart` — Creates a new presentation with a title; the searchable template picker + `templatePickerIcons`, hiding `requiresInfoSafety` templates (MIAUW) until the module is revealed. Outside Dutch it says so above the picker: a template's title and description run through `l10n.d()`, its example slides do not. That is a choice, not an omission — template content is deck content, and translating it would make a document's text depend on the menu language it happened to be created in. The same rule drives `_contentHomes` in `tool/check_hardcoded_text.dart`.
