...
- `marp_html_service.dart` — Builds the self-contained, sanitised HTML export with embedded assets. Parts: `parts/marp_html_service_charts*.dart` (charts to inline SVG), `parts/marp_html_service_cockpit.dart`, and `parts/marp_html_service_reporting*.dart` — the six table-backed reporting types (`scorecard`, `assets`, `discoveries`, `checklist`, `scope-matrix`, `findings-summary`), which would otherwise reach the recipient as a bare Markdown table while the app shows cards, bars and coverage counters. Slide images are inlined as `data:` URIs via an injected `HtmlImageResolver`; the service itself never reads a file, so it still builds on web. The stylesheet is split: `_structuralCss` always travels (timeline, sign-off, redaction block, TLP banner, print rules), the theme only adds colours and fonts — they used to *replace* each other, so a themed export lost all four.
