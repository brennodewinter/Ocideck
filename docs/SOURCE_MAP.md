...
- `markdown_service.dart` — Serializes decks to Marp markdown and parses it back (the file-format contract). `generateDeck` skips every slide with `renderPage > 0`: a render copy of a rich-text page has no shape in the file format — it carries the whole body and differs only in a field that is not serialized — so writing the expanded list out gave the same slide N times over, each time complete. The filter lives here rather than at the call site because the rule holds everywhere; `expandRichTextForRender` yields a list to *draw*, never one to write. Writes **no base64**: what is opaque, or is about the document instead of in it, lives in a sidecar. Part `parts/markdown_service_parse_front_matter.dart` holds the `--- … ---` header; `parts/markdown_service_parse_columns.dart` reads the visible `<h3>`/`<ul><li>` of a two-column slide, which is the content itself rather than a rendering of a comment above it. The reader takes a slide's type from the same registry the writer uses (`slideTypeByMarpClass` in `models/slide.dart`) instead of its own list of string literals.
...
- `privacy/privacy_checksums_eu.dart` — The European checksums: BE mod-97, DE ISO 7064 + digit-repetition, FR NIR, ES DNI/NIE, PT NIF, PL PESEL, IT codice fiscale, HR OIB, BG EGN, RO CNP, SE Luhn, FI mod-31, EE/LT mod-11, UK NHS/NINO, NL legacy VAT (elevenproof over the nine embedded digits, which are the owner's BSN) — with embedded-birthdate validation where the checksum alone is too weak.
...
- `gitea_forge.dart` — Forgejo/Gitea adapter (one adapter: same REST surface). The only place provider-specific knowledge may live. The release ops map onto the REST endpoints (`branches`, `tags`, `pulls`, `pulls/{n}/merge`) through the same `_apiUri`/`_headers`/`_checkStatus` skeleton. Gitea/Forgejo deliberately does **not** implement `CodeSearchCapable`: there is no REST code-search endpoint (go-gitea/gitea#31375).
...
- `deck_repo_serializer.dart` — `buildDeckRepoFiles`: turns a deck into its repo file set — `deck.md` bytes plus the missing image blobs, images rewritten `mem:`→`repo:`. Every reference of a slide goes through the pool, the fields and the `![…](…)` in the body alike; one left behind is a path in `deck.md` that only the author's machine resolves, and `AssetIndex` (which scans the raw text) then sees no reference at all. Pooling is async and rewriting is not, so it runs in two passes: resolve each reference, then rewrite the slide as a whole. The exact inverse of the open path's `repo:`→`mem:`; video/audio are reported, not written as broken refs. Chart data gets its own file next to `deck.md` at the path its `source` names (`chartDataFilesOf` writing, `withRepoChartData` reading) — deliberately not in the content-addressed pool, since a hash path turns every edited cell into a new file instead of a diff.
