...
- `rfc3161_timestamp.dart` — Builds a `.tsq` from the seal hash and reads a `.tsr` token far enough to compare its message imprint with that hash. That single comparison is the whole check: no CMS signature, no certificate chain, no nonce — see the file header for why not, and what that means for `genTime`.
