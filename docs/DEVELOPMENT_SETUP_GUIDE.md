...
- **Flutter 3.47.2** (stable) — the pinned version. Use the `dart` that
  ships inside it (3.13.2) rather than a separately installed one.
...
2. Extract to a permanent location:
   ```bash
   cd ~
   tar xf flutter_macos_3.47.2-stable.tar.xz  # macOS example
   ```
