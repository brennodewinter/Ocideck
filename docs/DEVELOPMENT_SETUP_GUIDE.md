...
- **Flutter 3.47.1** (stable) — the pinned version. Use the `dart` that
  ships inside it (3.13.1) rather than a separately installed one.
...
2. Extract to a permanent location:
   ```bash
   cd ~
   tar xf flutter_macos_3.47.1-stable.tar.xz  # macOS example
   ```
