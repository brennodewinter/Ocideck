---
marp: true
ocideck_format: 1
theme: ocideck
paginate: true
title: Doručte nabídku
language: cs
---

<!-- _class: title -->

# Doručte nabídku
## Pro koho to je · Na co se ptáte · Čas · Datum

---

# Cíl a základní sdělení

- Komu a proč dávám nabídku: …
- Hlavní sdělení v jedné větě: …
- Co konkrétně od nich žádám: …

---

<!-- _class: quote -->

> Pokud to nedokážete vysvětlit jednou větou, sami tomu ještě dostatečně nerozumíte.

— Základní pravidlo pro silnou prezentaci

---

# Struktura
<!-- ocideck_list_style: numbered -->

1. Háček: poutavý úvod nebo příklad
2. Problém: jakou bolest řešíte a pro koho
3. Řešení: co nabízíte, běžným jazykem
4. Důkaz: čísla, příklad nebo ukázka
5. Žádost: konkrétní další krok

---

<!-- _class: table table-editable -->

# Moje hřiště vyšlo

| Část | Co říkám |
| --- | --- |
| Háček | … |
| Problém | … |
| Řešení | … |
| Důkaz | … |
| Zeptejte se | … |

---

<!-- _class: table table-editable -->

# Předvídání otázek

| Očekávaná otázka | Moje odpověď |
| --- | --- |
| co to stojí? | … |
| proč ty? | … |
| Co když to nebude fungovat? | … |

---

# Připraveno k nadhazování
<!-- ocideck_list_style: checklist -->
<!-- ocideck_checklist_progress: true -->

- [ ] Základní zpráva a zeptejte se ostře
- [ ] Příběh nacvičený v časovém limitu
- [ ] Čísla a příklad připraveny
- [ ] Jasná výzva k akci
