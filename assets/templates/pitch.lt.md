---
marp: true
ocideck_format: 1
theme: ocideck
paginate: true
title: Pristatyti pitch'ą
language: lt
---

<!-- _class: title -->

# Pristatyti pitch'ą
## Kam skirta · Ko klausiate · Laikas · Data

---

# Tikslas ir pagrindinė žinutė

- Kam kreipiuosi ir kodėl jiems:…
- Pagrindinė žinutė vienu sakiniu:…
- Ko aš konkrečiai iš jų prašau:…

---

<!-- _class: quote -->

> Jei negalite to paaiškinti vienu sakiniu, patys dar nesate to pakankamai gerai supratę.

— Nykščio taisyklė stipriam pristatymui

---

# Struktūra
<!-- ocideck_list_style: numbered -->

1. Kabliukas: intriguojanti pradžia arba pavyzdys
2. Problema: kokį skausmą sprendžiate ir kam
3. Sprendimas: ką siūlote, paprasta kalba
4. Įrodymas: skaičiai, pavyzdys arba demonstracija
5. Prašymas: konkretus kitas žingsnis

---

<!-- _class: table table-editable -->

# Mano aikštelė pasiteisino

| dalis | Ką aš sakau |
| --- | --- |
| Kabliukas | … |
| Problema | … |
| Sprendimas | … |
| Įrodymas | … |
| Paklausk | … |

---

<!-- _class: table table-editable -->

# Numatyti klausimai

| Laukiamas klausimas | Mano atsakymas |
| --- | --- |
| Kiek tai kainuoja? | … |
| Kodėl tu? | … |
| Ką daryti, jei tai neveikia? | … |

---

# Pasiruošę pristatyti
<!-- ocideck_list_style: checklist -->
<!-- ocideck_checklist_progress: true -->

- [ ] Pagrindinė žinutė ir paklauskite aštriai
- [ ] Istorija repetuota per nustatytą laiką
- [ ] Skaičiai ir pavyzdys paruošti
- [ ] Aiškus raginimas veikti
