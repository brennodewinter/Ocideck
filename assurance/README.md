...
| [`app-store-distributie-positie.md`](app-store-distributie-positie.md) | De kernwaardentoets (`bewaker`-lijn) op distributie via de Apple Mac App Store en de Microsoft Store. Uitkomst: directe download blijft canoniek; Apple-store afgewezen (sandbox breekt git-opslag, review remt beveiligingsfixes), Microsoft-store aanvaardbaar als nevenkanaal (full-trust MSIX, geen amputatie). Raakt de heroverwegingsconditie in `CRA-2024-2847-positie.md`. |
| [`soevereiniteit-ecsf.md`](soevereiniteit-ecsf.md) | OciDeck langs de acht doelen van het European Cloud Sovereignty Framework (ECSF, boek *November*). Consultancy-niveau zelfpositionering (een inschatting op grond van de code en documentatie hier — géén audit), per doel top-down getoetst aan de niveaudefinities. Uitkomst: niveau 4 op zes van de acht doelen; SOV-5 op 3 (Flutter/Dart is BSD-open en forkbaar → hoge overstapkosten maar géén intrekkingsrisico), SOV-7 op 3 (zachtste inschatting, hard te maken met een externe toets), SOV-8 n.v.t. (schaal veronderstelt een datacenter). Twee sturende inzichten voor #1227: kanalen zijn additief (verlagen de bodem niet) en méér routes verhógen de operationele soevereiniteit (SOV-4) — waarmee Snap als één-van-velen alsnog verdedigbaar is. |
