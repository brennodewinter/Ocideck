...
| [`soevereiniteit-ecsf.md`](soevereiniteit-ecsf.md) | OciDeck langs de acht doelen van het European Cloud Sovereignty Framework (ECSF, boek *November*). Consultancy-niveau zelfpositionering, per doel én per distributieroute getoetst aan de niveaudefinities. Kern hoog (lokaal-eerst, open code én formaat); verdiepingen op SOV-5 (Flutter/Dart is BSD-open en forkbaar → hoge overstapkosten maar géén intrekkingsrisico, niveau 3), SOV-7 (geen formele externe verificatie → begrensd op 2) en SOV-8 (datacenter-schaal n.v.t. voor een lokaal product). Twee sturende inzichten voor #1227: kanalen zijn additief (verlagen de bodem niet) en méér routes verhógen de operationele soevereiniteit (SOV-4) — waarmee Snap als één-van-velen alsnog verdedigbaar is. |
